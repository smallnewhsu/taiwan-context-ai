...
數產署開源的Taiwan Tongues ASR CE提供國語、台語、客語及英語預訓練模型。本專案保留上述辨識路徑，並針對台灣新住民家庭、照護及公共服務場域的跨語需求，新增越南語辨識與繁體中文字面翻譯流程。

|比較項目|Taiwan Tongues ASR CE|Taiwan Context AI|
|---|---|---|
|語言範圍|國語、台語、客語、英語|保留原有語言，另增加越南語|
|越南語辨識|未提供預訓練推論模型|以faster-whisper建立獨立越南語辨識路徑|
|語言選擇|依原模型服務設定|可明確選擇國語、台語、客語、英語、混合語音或越南語|
|繁中輸出|以語音逐字稿為主|越南語逐字稿另產生繁體中文字面意思|
|語境理解|以語音辨識為核心|加入人物關係、生活情境、可能意境、判斷依據及建議回應|
|安全機制|依原始ASR輸出|翻譯空白、照抄或資訊不足時要求使用者確認|
|多模態整合|語音辨識服務|整合文字改寫、影像理解及影像後接語音問答|

越南語流程使用faster-whisper進行辨識，再由Gemma 3 4B產生繁體中文字面意思，最後交由Taiwan Context Engine進行語境分析與安全確認。此項成果屬系統整合與推論流程擴充，未宣稱對Taiwan Tongues ASR CE進行越南語模型權重微調。

目前越南語功能已完成端到端流程驗證；獨立多說話者測試集仍在建置，因此尚不宣稱越南語辨識準確率或正式服務品質。
